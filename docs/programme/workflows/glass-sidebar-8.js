export const meta = {
  name: "glass-sidebar-8",
  description:
    "Phase 2d (Codex-only): P7 sticky chrome fades into the pane instead of a solid band; gated; reinstall; audit.",
  phases: [
...
const p7 = await produceAndGate(PACKETS.P7, prior);
const releaseList = "Prior phases: RELEASED and live\nP7: " + (p7.released ? "RELEASED" : "HELD");

const integration = await callWorker(
  "sol-high",
  "You are the integration worker for P7.\n" + COMMON + "\n\n" +
    "If P7 is RELEASED: in /home/system/workspaces/LAL/Development/bb-plugins/liquid-glass run `npm test`, `npx tsc --noEmit`, `bb plugin build`, then `bb plugin install /home/system/workspaces/LAL/Development/bb-plugins/liquid-glass --yes` (path reinstall; it does not change the active theme or any appearance value), confirm the new version is running with a clean log tail, and add a 'Phase 2d' paragraph to " + PLANS + "/HANDOFF.md. If HELD, do nothing and say so.\n" +
    "----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(p7) + "\n----- END -----\n" +
...
const finalReview = await callWorker(
  "codex-final",
  "You are the auditor for P7.\n" + COMMON + " You must not edit any file.\n\n" +
    "Read " + PLANS + "/packet-P7-chrome-fade.md and the report below; in bb-plugins/liquid-glass run `npm test` and `npx tsc --noEmit`; grep both theme css files for the header/dock selectors and confirm gradient backgrounds with --lg-chrome-a / --lg-chrome-fade and a mask-image on the blur, and that popover/menu/dialog selectors still use overlayOpacity; run `bb plugin list | grep -A1 liquid-glass`.\n" +
    "----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(p7) + "\n----- INTEGRATION -----\n" + integration + "\n----- END -----\n" +
...
return { releases: [{ id: "P7", released: p7.released, rounds: p7.rounds }], integration, finalReview, review: p7.review };
