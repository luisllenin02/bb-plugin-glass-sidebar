export const meta = {
  name: "glass-sidebar-13",
  description:
    "Phase 2g (Codex-only): P7b masks the chrome background layer instead of the content; gated; reinstall; audit.",
  phases: [
...
const PACKETS = {
  ...
  P7b: { id: "P7b", title: "Theme: chrome fade on a background layer only", file: PLANS + "/packet-P7b-chrome-fade-fix.md", producer: "sol-high" },
...
const pk = await produceAndGate(PACKETS.P7b, args && args.priorAppendix ? String(args.priorAppendix) : "");
const releaseList = "Prior phases: RELEASED and live\nP7b: " + (pk.released ? "RELEASED" : "HELD");
const integration = await callWorker("sol-high",
  "You are the integration worker for P7b.\n" + COMMON + "\n\nIf P7b is RELEASED: in /home/system/workspaces/LAL/Development/bb-plugins/liquid-glass run `npm test`, `npx tsc --noEmit`, `bb plugin build`, `bb plugin install /home/system/workspaces/LAL/Development/bb-plugins/liquid-glass --yes` (path reinstall; does not change the active theme or any appearance value), confirm the new version is running with a clean log tail; add a 'Phase 2g' paragraph to " + PLANS + "/HANDOFF.md. If HELD, do nothing and say so.\n----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(pk) + "\n----- END -----\nYour final message is the report in the brief's section 5 shape.",
  "integrate:I1", "Integrate");
const finalReview = await callWorker("codex-final",
  "You are the auditor for P7b.\n" + COMMON + " You must not edit any file.\n\nRead " + PLANS + "/packet-P7b-chrome-fade-fix.md and the reports below. In /home/system/workspaces/LAL/Development/bb-plugins/liquid-glass run `npm test` and `npx tsc --noEmit`; grep both theme css files: mask-image must appear only in ::before rules for chrome selectors, dock gradient direction to top, header to bottom, composer card untouched by masks; run `bb plugin list | grep -A1 liquid-glass`.\n----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(pk) + "\n----- INTEGRATION -----\n" + integration + "\n----- END -----\nReturn the structured verdict through the workflow result tool exactly once.",
  "final-review", "Final review", reviewSchemaWide);
return { releases: [{ id: "P7b", released: pk.released, rounds: pk.rounds }], integration, finalReview, review: pk.review };
