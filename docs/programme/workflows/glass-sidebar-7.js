export const meta = {
  name: "glass-sidebar-7",
  description:
    "Phase 2c (Codex-only): P5b shows running workflows as child rows (sidebar) in parallel with P6 mobile sidebar glass (theme); each gated; integrate; audit.",
  phases: [
...
const [p5b, p6] = await parallel([
  () => produceAndGate(PACKETS.P5b, prior),
  () => produceAndGate(PACKETS.P6, ""),
]);
const results = [p5b, p6].filter(Boolean);
const releaseList = "Phase 1, 2, 2b: RELEASED and live\n" + results.map((r) => r.id + ": " + (r.released ? "RELEASED" : "HELD")).join("\n");
const appendixAll = results.map(summarize).join("\n\n");

const integration = await callWorker(
  "sol-high",
  "You are the integration worker for P5b and P6.\n" + COMMON + "\n\n" +
    "Sidebar (if P5b RELEASED): in /home/system/workspaces/LAL/Development/forks/bb-sidebar confirm a clean tree with the [P5b] commit, run `npx tsc --noEmit` and `npx vitest run --config vitest.config.ts`, `bb plugin build`, `bb plugin reload bb-sidebar`, confirm running with a clean `bb plugin logs bb-sidebar -n 40` tail (there must be no periodic log lines and no background service registered for workflow activity), run `python3 /home/system/workspaces/LAL/Development/forks/check-plugin-slot-conflicts.py` (exit 0). Theme (if P6 RELEASED): in /home/system/workspaces/LAL/Development/bb-plugins/liquid-glass run `npm test`, `npx tsc --noEmit`, `bb plugin build`, then `bb plugin install /home/system/workspaces/LAL/Development/bb-plugins/liquid-glass --yes` (path reinstall refreshes version metadata; it does not change the active theme) and confirm liquid-glass 0.5.1 running. Then add a 'Phase 2c' paragraph to " + PLANS + "/HANDOFF.md. Do not `bb theme set` and do not change any liquid-glass appearance value.\n" +
    "----- RELEASE LIST -----\n" + releaseList + "\n----- REPORTS -----\n" + appendixAll + "\n----- END -----\n" +
...
const finalReview = await callWorker(
  "codex-final",
  "You are the auditor for phase 2c.\n" + COMMON + " You must not edit any file.\n\n" +
    "Read " + PLANS + "/packet-P5b-workflow-signal.md and " + PLANS + "/packet-P6-mobile-sidebar-glass.md and the reports below. Sidebar: in forks/bb-sidebar run `npx tsc --noEmit`, the vitest suite, `git log --oneline main..HEAD`, read `git diff d49b781..HEAD -- src/WorkflowRunRow.tsx src/workflow-activity.ts src/SlimRow.tsx` (skip missing), and verify the data source is a read-only query of the workflows plugin database with no background service, interval, or CLI spawn on the server (grep server.ts for setInterval, bb.background, child_process, execFile: all must be absent for this feature); confirm the RPC would list this very run (origin thread thr_3uy6dx4vfp) by running the same SELECT against /home/system/.bb/plugins/workflows/data.db read-only. Theme: in bb-plugins/liquid-glass run `npm test` and grep both css files' compact media blocks for any sidebar alpha override. Judge both packets against their acceptance points and the Preservation rule.\n" +
    "----- RELEASE LIST -----\n" + releaseList + "\n----- REPORTS -----\n" + appendixAll + "\n----- INTEGRATION -----\n" + integration + "\n----- END -----\n" +
