...
import { RelatedThreadTree } from "./RelatedThreadTree";
import {
...
export interface ThreadReorderControls {
  disabled: boolean;
  isDragging: boolean;
  /** False for pointer-only organization drags on a sorted flat shelf. */
  hasKeyboardReorder?: boolean;
...
export function ThreadCard({
  ...
  /** Folder and accent actions are supplied by the inbox's single org hook. */
  ...
  const hasWorkflow =
    ...
    relatedWorkflowRuns.some((run) => run.originThreadId === thread.id);
  const hasActivity =
    hasWorkflow || Object.values(thread.activity).some((count) => count > 0);
  const canParkNow = canPark && !hasActivity;
  const emphasis = isWoke
    ? "woke"
    : thread.isUnread
      ? "unread"
      : thread.indicator === "none" && !hasActivity
        ? "read-idle"
        : "active";
  ...
  return (
    <RowContextMenu
      thread={thread}
      canSnooze={canParkNow}
      canArchive={canParkNow}
      snoozePresets={snoozePresets}
      onSnooze={onSnooze}
      onSettle={canParkNow ? onSettle : undefined}
          ...
          <div className="pointer-events-none relative mt-0.5 flex h-4 items-center gap-1.5 text-2xs text-muted-foreground">
            ...
            {thread.activity.backgroundCommands > 0 ? (
              <ActivityCount
                ...
                count={thread.activity.backgroundCommands}
              />
            ) : null}
            {thread.activity.planMode > 0 ? (
              <ActivityCount label="plans" count={thread.activity.planMode} />
            ) : null}
            {thread.activity.goals > 0 ? (
              <ActivityCount label="goals" count={thread.activity.goals} />
            ) : null}
            {relatedCount > 0 ? (
              <button
                type="button"
                ...
                className="pointer-events-auto relative z-10 ml-auto flex shrink-0 items-center gap-0.5 rounded px-1 text-2xs text-muted-foreground hover:bg-sidebar-accent hover:text-foreground"
              >
                <Icon
                  name={relatedExpanded ? "ChevronDown" : "ChevronRight"}
                  className="size-3"
                  aria-hidden
                />
                {relatedCount}
