...
describe("legacy scaffold columns", () => {
  // Q0 created `thread_lifecycle` with NOT NULL `state` and `updated_at`.
  // Every write mirrors the fork's five fields into those columns, so the
  // constraint is always satisfied and no reader ever treats them as truth.
  it("pins the state over all four input shapes", () => {
    expect(legacyLifecycleState(row())).toBe("active");
    expect(legacyLifecycleState(row({ snoozedUntil: 2_000, snoozedAt: 500 }))).toBe(
      "snoozed",
    );
    expect(legacyLifecycleState(row({ settledAt: 500 }))).toBe("settled");
    // Settle clears a snooze, so both set at once is a policy settle laid
    // over a live snooze: settled is what a legacy reader must see.
    expect(
      legacyLifecycleState(
        row({ settledAt: 500, snoozedUntil: 2_000, snoozedAt: 400 }),
      ),
    ).toBe("settled");
  });

  it("treats an explicit settled override as settled and active as active", () => {
    expect(legacyLifecycleState(row({ settledOverride: "settled" }))).toBe(
      "settled",
    );
    expect(legacyLifecycleState(row({ settledOverride: "active" }))).toBe(
      "active",
    );
  });

  it("mirrors the snooze wake time and stamps the write clock", () => {
    expect(legacyWakeAt(row({ snoozedUntil: 2_000, snoozedAt: 500 }))).toBe(2_000);
    expect(legacyWakeAt(row({ settledAt: 500 }))).toBeNull();
    expect(
      legacyLifecycleColumns(row({ snoozedUntil: 2_000, snoozedAt: 500 }), 7_777),
    ).toEqual({ state: "snoozed", wakeAt: 2_000, updatedAt: 7_777 });
  });
});
