export const meta = {
  name: "glass-sidebar-10",
  description:
    "Phase 2c-fix (Codex-only): P5c makes the workflow-row work buildable, gated; rebuild and reload the sidebar; audit.",
  phases: [
...
const p5c = await produceAndGate(PACKETS.P5c, args && args.priorAppendix ? String(args.priorAppendix) : "");
const releaseList = "Prior phases: RELEASED and live\nP5c: " + (p5c.released ? "RELEASED" : "HELD");
const integration = await callWorker(
  "sol-high",
  "You are the integration worker for P5c.\n" + COMMON + "\n\n" +
    "If P5c is RELEASED: in /home/system/workspaces/LAL/Development/forks/bb-sidebar confirm a clean tree with the [P5c] commit, run `npx tsc --noEmit`, `npx vitest run --config vitest.config.ts`, `bb plugin build` (must succeed), `bb plugin reload bb-sidebar`, confirm running with a clean `bb plugin logs bb-sidebar -n 40` tail, run `python3 /home/system/workspaces/LAL/Development/forks/check-plugin-slot-conflicts.py` (exit 0), and add a 'Phase 2c (workflow rows live)' paragraph to " + PLANS + "/HANDOFF.md. If HELD, do nothing and say so.\n" +
    "----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(p5c) + "\n----- END -----\n" +
...
const finalReview = await callWorker(
  "codex-final",
  "You are the auditor for P5c.\n" + COMMON + " You must not edit any file.\n\n" +
    "Read " + PLANS + "/packet-P5c-build-fix.md and the reports below. In forks/bb-sidebar: `git log --oneline -3`, `npx tsc --noEmit`, the vitest suite, `bb plugin build`, and `bb plugin list | grep -A1 bb-sidebar`; confirm dist/app.js contains no better-sqlite3 or node: import (grep), and that the read-only workflow query still lists a running run when one exists.\n" +
    "----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(p5c) + "\n----- INTEGRATION -----\n" + integration + "\n----- END -----\n" +
    "Return the structured verdict through the workflow result tool exactly once.",
  "final-review", "Final review", reviewSchemaWide,
);
return { releases: [{ id: "P5c", released: p5c.released, rounds: p5c.rounds }], integration, finalReview, review: p5c.review };
