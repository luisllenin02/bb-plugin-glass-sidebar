export const meta = {
  name: "glass-sidebar-2",
  description:
    "Run 2: gate the already-produced B1 state layer (report passed in args), then B3 folders, B4 live strip, integration, and the final Fable audit.",
  phases: [
...
async function produceAndGate(p, appendix) {
  ...
  let review = await callWorker("fable-high", reviewPrompt(p, report, 0), "review:" + p.id, "Review", reviewSchemaWide);
  let rounds = 0;
  while (review && review.verdict !== "pass" && rounds < 2) {
    ...
    review = await callWorker("fable-high", reviewPrompt(p, report, rounds), "re-review:" + p.id + "#" + rounds, "Review", reviewSchemaWide);
  }
...
const priorAppendix = args && args.priorAppendix ? String(args.priorAppendix) : "";
const b1 = await gateOnly(PACKETS.B1, args && args.b1Report ? String(args.b1Report) : "(no producer report was captured; review the committed state on the branch)");

let b3 = null;
if (b1.released) {
  b3 = await produceAndGate(PACKETS.B3, priorAppendix + "\n" + summarize(b1));
} else {
  log("B3 skipped: B1 not released");
}

let b4 = null;
if (b3 && b3.released) {
  b4 = await produceAndGate(PACKETS.B4, priorAppendix + "\n" + summarize(b3));
} else {
  log("B4 skipped: B3 not released");
}

const results = [b1, b3, b4].filter(Boolean);
const releaseList = (args && args.priorReleases ? String(args.priorReleases) + "\n" : "") +
  results.map((r) => r.id + ": " + (r.released ? "RELEASED" : "HELD")).join("\n");
const appendixAll = priorAppendix + "\n\n" + results.map(summarize).join("\n\n");
