export const meta = {
  name: "glass-sidebar-6",
  description:
    "Phase 2c (Codex-only): P5b closes the two workflow-signal blockers in the sidebar fork, gated by review, then reload and a bounded audit.",
  phases: [
...
const p5b = await produceAndGate(PACKETS.P5b, prior);
const releaseList = "Phase 1, 2, 2b: RELEASED and live\nP5b: " + (p5b.released ? "RELEASED" : "HELD");

const integration = await callWorker(
  "sol-high",
  "You are the integration worker for P5b.\n" + COMMON + "\n\n" +
    "If P5b is RELEASED: in /home/system/workspaces/LAL/Development/forks/bb-sidebar confirm a clean tree with the [P5b] commit, run `npx tsc --noEmit` and `npx vitest run --config vitest.config.ts`, `bb plugin build`, `bb plugin reload bb-sidebar`, confirm `bb plugin list` shows bb-sidebar running and `bb plugin logs bb-sidebar -n 40` has no errors (one workflow-activity change line is fine), run `python3 /home/system/workspaces/LAL/Development/forks/check-plugin-slot-conflicts.py` (exit 0), then add a 'Phase 2c' paragraph to " + PLANS + "/HANDOFF.md describing the workflow signal and its source. If HELD, do nothing and say so.\n" +
    "----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT AND REVIEW -----\n" + summarize(p5b) + "\n----- END -----\n" +
...
const finalReview = await callWorker(
  "codex-final",
  "You are the auditor for P5b.\n" + COMMON + " You must not edit any file.\n\n" +
    "Read " + PLANS + "/packet-P5b-workflow-signal.md and the report below. In /home/system/workspaces/LAL/Development/forks/bb-sidebar run `npx tsc --noEmit`, the vitest suite, `git log --oneline main..HEAD`, and read `git diff d49b781..HEAD -- src/server.ts src/useWorkflowActivity.ts src/SlimRow.tsx` (skip missing). Then check live evidence: this very run is a workflow from origin thread thr_3uy6dx4vfp; call the fork's RPC through `bb plugin logs bb-sidebar -n 40` (a workflow-activity change line) or inspect the plugin database/kv if the poller stores a snapshot; state whether thr_3uy6dx4vfp is reported as running. Judge both blockers closed, preservation held (suite ≥ 296 and only grew), and the plugin running.\n" +
    "----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(p5b) + "\n----- INTEGRATION -----\n" + integration + "\n----- END -----\n" +
...
return { releases: [{ id: "P5b", released: p5b.released, rounds: p5b.rounds }], integration, finalReview, review: p5b.review };
