export const meta = {
  name: "glass-sidebar-12",
  description:
    "Phase 2f (Codex-only): P8b matter-specific icons and the open-picker request in Project Icons; gated; reinstall; audit.",
  phases: [
...
const PACKETS = {
  ...
  P8b: { id: "P8b", title: "Project Icons: matter-specific icons + open-picker request", file: PLANS + "/packet-P8b-matter-icons.md", producer: "sol-xhigh" },
...
const pk = await produceAndGate(PACKETS.P8b, args && args.priorAppendix ? String(args.priorAppendix) : "");
const releaseList = "Prior phases: RELEASED and live\nP8b: " + (pk.released ? "RELEASED" : "HELD");
const integration = await callWorker("sol-high",
  "You are the integration worker for P8b.\n" + COMMON + "\n\nIf P8b is RELEASED: in /home/system/workspaces/LAL/Development/.bb/vendor/bb-plugins-ariofrio-39a8cf1a/plugins/bb-plugin-project-icons confirm a clean branch feat/auto-icon-colour with the [P8b] commit, run the tests and `npx tsc --noEmit`, `bb plugin build`, `bb plugin install /home/system/workspaces/LAL/Development/.bb/vendor/bb-plugins-ariofrio-39a8cf1a/plugins/bb-plugin-project-icons --yes`, confirm project-icons running with a clean log tail, then read the project_icon rows read-only with a short node script and report how many matter projects now carry a non-balance-scale icon; add a 'Phase 2f' paragraph to " + PLANS + "/HANDOFF.md. If HELD, do nothing and say so.\n----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(pk) + "\n----- END -----\nYour final message is the report in the brief's section 5 shape.",
  "integrate:I1", "Integrate");
const finalReview = await callWorker("codex-final",
  "You are the auditor for P8b.\n" + COMMON + " You must not edit any file.\n\nRead " + PLANS + "/packet-P8b-matter-icons.md and the reports below. In /home/system/workspaces/LAL/Development/.bb/vendor/bb-plugins-ariofrio-39a8cf1a/plugins/bb-plugin-project-icons run the tests and `npx tsc --noEmit`; read auto-assign.ts (or the classifier module) and its fixtures; verify every family export exists in icon-catalog.json; verify reading caps; verify the open-picker broadcast handler and that no model call or timer exists; run `bb plugin list | grep -A1 project-icons`.\n----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(pk) + "\n----- INTEGRATION -----\n" + integration + "\n----- END -----\nReturn the structured verdict through the workflow result tool exactly once.",
  "final-review", "Final review", reviewSchemaWide);
return { releases: [{ id: "P8b", released: pk.released, rounds: pk.rounds }], integration, finalReview, review: pk.review };
