 ...
 *
 * Ported from the bb-sidebar fork (yusuf8834/bb-sidebar lineage) unchanged,
 * plus the legacy-column mirror this tree's `thread_lifecycle` table needs.
 */

// Q1's `row-props.ts` is the canonical declaration of the preset shape; this
// module re-exports it so the structural type keeps exactly one definition.
export type { ConfiguredSnoozePreset } from "./row-props";
import type { ConfiguredSnoozePreset } from "./row-props";

/** Channel every client re-reads the lifecycle store on. */
...
/**
 * The scaffold's `thread_lifecycle` table (Q0) declares `state TEXT NOT NULL`,
 * `wake_at` and `updated_at NOT NULL`. The ported code reads and writes the
 * fork's five fields instead, so those three columns are kept as derived
 * mirrors: every write supplies them, and nothing reads them back as truth.
 * SQLite cannot relax a NOT NULL constraint with `ALTER TABLE` and recreating
 * the table is forbidden, so the reconciliation is additive on the schema and
 * total on the writes.
 */
export type LegacyLifecycleState = "active" | "snoozed" | "settled";

/**
 * Settled wins over snoozed here on purpose: `settle` clears any snooze, so a
 * row carrying both is a policy settle laid over a live snooze, and "settled"
 * is the state a legacy reader should see.
 */
export function legacyLifecycleState(
  row: ThreadLifecycleRow,
): LegacyLifecycleState {
  if (row.settledAt !== null || row.settledOverride === "settled") {
    return "settled";
  }
  if (row.snoozedUntil !== null) return "snoozed";
  return "active";
}

/** The legacy wake column is the snooze wake time, or null when not snoozed. */
export function legacyWakeAt(row: ThreadLifecycleRow): number | null {
  return row.snoozedUntil;
}

/** The three mirrored NOT NULL / nullable scaffold columns for one write. */
export function legacyLifecycleColumns(
  row: ThreadLifecycleRow,
  now: number,
): { state: LegacyLifecycleState; wakeAt: number | null; updatedAt: number } {
  return {
    state: legacyLifecycleState(row),
    wakeAt: legacyWakeAt(row),
    updatedAt: now,
  };
}
