export const meta = {
  name: "glass-sidebar-14",
  description:
    "Phase 2h (Codex-only): P9 sidebar project glyph mirrors the header and the row context menu opens the Project Icons picker; gated; reload; audit.",
  phases: [
...
const PACKETS = {
  ...
  P9: { id: "P9", title: "Sidebar: header-mirrored glyph and project icon menu", file: PLANS + "/packet-P9-sidebar-icon-menu.md", producer: "sol-high" },
...
const pk = await produceAndGate(PACKETS.P9, args && args.priorAppendix ? String(args.priorAppendix) : "");
const releaseList = "Prior phases: RELEASED and live\nP9: " + (pk.released ? "RELEASED" : "HELD");
const integration = await callWorker("sol-high",
  "You are the integration worker for P9.\n" + COMMON + "\n\nIf P9 is RELEASED: in /home/system/workspaces/LAL/Development/forks/bb-sidebar confirm a clean tree with the [P9] commit, run `npx tsc --noEmit`, the vitest suite, `bb plugin build`, `bb plugin reload bb-sidebar`, confirm running with a clean log tail and no 'rpc input validation failed' note in `bb plugin list`, run the slot-conflict guard (exit 0), and add a 'Phase 2h' paragraph to " + PLANS + "/HANDOFF.md. If HELD, do nothing and say so.\n----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(pk) + "\n----- END -----\nYour final message is the report in the brief's section 5 shape.",
  "integrate:I1", "Integrate");
const finalReview = await callWorker("codex-final",
  "You are the auditor for P9.\n" + COMMON + " You must not edit any file.\n\nRead " + PLANS + "/packet-P9-sidebar-icon-menu.md and the reports below. In /home/system/workspaces/LAL/Development/forks/bb-sidebar run `npx tsc --noEmit`, the vitest suite, `git log --oneline -3`; read src/RowContextMenu.tsx diff since the [P5d] commit; curl the getProjectDecor RPC (POST '{}' to http://127.0.0.1:38886/api/v1/plugins/bb-sidebar/rpc/getProjectDecor) and require non-empty projects; run `bb plugin list | grep -A1 bb-sidebar` and require no rpc error note.\n----- RELEASE LIST -----\n" + releaseList + "\n----- REPORT -----\n" + summarize(pk) + "\n----- INTEGRATION -----\n" + integration + "\n----- END -----\nReturn the structured verdict through the workflow result tool exactly once.",
  "final-review", "Final review", reviewSchemaWide);
return { releases: [{ id: "P9", released: pk.released, rounds: pk.rounds }], integration, finalReview, review: pk.review };
