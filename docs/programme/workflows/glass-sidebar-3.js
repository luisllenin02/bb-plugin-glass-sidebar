export const meta = {
  name: "glass-sidebar-3",
  description:
    "Run 3 (Codex-only continuation): finish B3 gating, B4 live strip, integration, and the final audit on Codex because Claude usage limits are close.",
  phases: [
...
async function produceAndGate(p, appendix) {
  ...
  let review = await callWorker(p.id === "B3" ? "fable-high" : "codex-review", reviewPrompt(p, report, 0), "review:" + p.id, "Review", reviewSchemaWide);
  let rounds = 0;
