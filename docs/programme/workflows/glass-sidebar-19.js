export const meta = {
  name: "glass-sidebar-19",
  description:
    "Phase 4d: repair a held packet from a prior run (args.repair = {id, report, review}), gate it, then continue the port waves (args.waves, e.g. [[\"Q1\",\"Q2\",\"Q4\"],[\"Q3\",\"Q5\",\"Q6\"]]); Codex gates; Codex audit at the end; no install unless a packet says so.",
  phases: [
...
const waves = (args && Array.isArray(args.waves) && args.waves.length) ? args.waves : [["Q2", "Q3"], ["Q4", "Q5", "Q6"]];
let appendix = (args && args.priorAppendix) ? String(args.priorAppendix) : "";
log("Budget: " + JSON.stringify(budget()) + " waves: " + JSON.stringify(waves));
const results = [];

// Repair-first: a packet held by a prior run gets one more repair with the
// exact blocking list, then a fresh gate (and up to two more rounds).
async function repairAndGate(p, priorReport, priorReview) {
  let report = await callWorker(p.producer, repairPrompt(p, priorReport, priorReview, 3), "repair:" + p.id + "#3", "Repair");
  if (!report) report = priorReport;
  let review = await callWorker("codex-review", reviewPrompt(p, report, 3), "re-review:" + p.id + "#3", "Review", reviewSchema);
  let rounds = 3;
  while (review && review.verdict !== "pass" && rounds < 5) {
    rounds += 1;
    log("Packet " + p.id + ": review failed with " + review.blocking.length + " blocking items; repair round " + rounds);
    const repaired = await callWorker(p.producer, repairPrompt(p, report, review, rounds), "repair:" + p.id + "#" + rounds, "Repair");
    if (repaired) report = repaired;
  ...
  const released = Boolean(review && review.verdict === "pass");
  log("Packet " + p.id + ": " + (released ? "RELEASED" : "HELD") + " after repair round " + rounds);
  return { id: p.id, released, report, review, rounds };
}

let repairHeld = false;
if (args && args.repair && args.repair.id) {
  const p = PACKETS[args.repair.id];
  if (!p) throw new Error("unknown packet " + args.repair.id);
  const r = await repairAndGate(p, String(args.repair.report || ""), args.repair.review || { verdict: "fail", blocking: [], nonBlocking: [], summary: "", checks: {} });
  results.push(r);
  appendix += "\n" + summarize(r);
  repairHeld = !r.released;
}
if (repairHeld) log("Repaired packet still HELD; port waves are skipped");
for (const wave of (repairHeld ? [] : waves)) {
